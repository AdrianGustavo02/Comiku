...
export default function ChatView({ channel, authUser }) {
  ...
  async function refreshChannelData(currentChannel = activeChannel) {
  ...
  useEffect(() => {
    ...
    // Enrich channel with Firestore data
    void refreshChannelData(channel)
  }, [channel])
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: '#64748b' }}>
            {activeChannel?.data?.members?.length || 0} miembros
          </div>
        </div>
      </div>
    )
        ...
        <Channel channel={activeChannel} doConnect={false}>
          <Window>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #e6e6e6' }}>
              {isGroupChat ? <GroupHeader /> : <ChannelHeader />}
              {isGroupChat && (
                <button
                  type="button"
